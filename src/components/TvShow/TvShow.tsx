...
import { selectTvShow, selectTvShowLoading } from './tvShowSlice.ts';

const TvShow = () => {
  const { id } = useParams();
  const dispatch = useAppDispatch();
  const tvShow = useAppSelector(selectTvShow);
  const tvShowLoading = useAppSelector(selectTvShowLoading);
  ...
  return (
   <>
     {tvShowLoading? <Box textAlign='center' marginTop={5}><CircularProgress /> </Box>: <Box component="div" marginTop={5}>
     {tvShow && (
